...
**Última atualização:** 2026-09-10 (9ª atualização do dia — ordem final das seções/menu + posicionamento marketing/desenvolvimento; alteração consciente de escopo)
**Fase atual:** Fase 4 — Limpeza da autoavaliação de nível (**construída no branch `rascunho`**, aguardando conferência visual do autor)
**Próximo passo recomendado:** o autor confere no Live Server o branch `rascunho` (ordem da página e do menu: **Sobre → Projetos → Tecnologias → Contato**; a seção `#sobre` abre a página, logo após a apresentação; cards "formação" e "objetivo" com Marketing ao lado de ADS; `#tecnologias` só com "Conteúdo em preparação." até a Fase 7) nos dois temas e, se possível, no celular. Estando OK, o fluxo de publicação (push do `rascunho`, merge em `main`, push do `main`, retorno ao `rascunho`) é executado. Depois começa a **Fase 5 — Imagens de tela e ampliação ao toque**. Ver `docs/PLANO.md`.

**Alteração consciente de escopo (2026-09-10, decisão do autor):** (1) `#sobre` volta para **antes de `#projetos`** e o menu passa a `Sobre · Projetos · Tecnologias · Contato` — contraria o FSD (1, 5.3, 12.1, 13, 25), que foi atualizado; motivo: dar contexto a quem chega sem repertório. (2) Posicionamento passa a cobrir **marketing e desenvolvimento** — Marketing (UNIBRA, 2020) ao lado de ADS na seção Sobre; "objetivo" serve às duas frentes — contraria o FSD (6/A1, 6/D1, 10, 26.5), que foi atualizado. Ver a entrada no histórico. Não abre nem fecha fase.
