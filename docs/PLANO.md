...
- [x] **Ajustes da conferência do autor:** imagens dos cards e do processo em **miniatura de 150px** (flex-wrap) que expandem na sobreposição ao clique — antes ocupavam a largura da coluna e ficavam altas demais no celular (altera o visual da Fase 5, já publicada; ajuste consciente, mesmo portão de publicação); linha `.nota-ficticia` ("dados fictícios, só para demonstração") abaixo de cada grupo de imagens (2 cards + processo).
- [x] **Ajustes pós-publicação (pedido do autor):** botão "Pedir a gravação" **de volta à esquerda** (a centralização de 2026-09-10 foi revertida — regra removida do bloco "FASE 6" do CSS); cada card de projeto passou de 2 para **4 imagens** (`hgs-3/4.png` e `hfs-3/4.png` tratados igual à Fase 5 — recorte 150/232, WebP q82, 1170×2150, 39–62 KB — em `imagens/hgs/checklist-de-limpeza.webp`, `imagens/hgs/tarefas-do-dia.webp`, `imagens/hfs/despesas-e-receitas-do-mes.webp`, `imagens/hfs/painel-do-mes.webp`), fechando o padrão de 4 recortes por seção. Conferência FSD 21.4 feita — sem dado de hóspede, sem nome da pousada, sem credencial, barra do navegador recortada. `.projeto-imagem` 12× no total; nenhuma regra CSS nova (o `flex-wrap` acomoda os 4).
...
- [~] Adicionar as media queries de `768px` e `480px` conforme a tabela de comportamento por componente (12.8): `.hero-content` empilha, avatar reduz a `110px`; `h1` `2.8em → 2.2em → 1.9em`; grades de duas colunas passam a uma; paddings laterais `40px → 24px → 16px`; paddings verticais de seção `48px → 32px → 24px`; menu `gap 32px → 16px → 12px` com quebra em duas linhas em vez de estourar. **Parte já feita em 2026-09-10 (ajuste fora de fase, a pedido do autor — ver `docs/STATUS.md`):** bloco de media queries `768px`/`480px` no fim de `css/estilo.css` cobrindo hero (empilha, foto acima do texto, avatar `130px`/`110px`, `h1` `2.2em`/`1.9em`), menu (`gap`/fonte/`flex-wrap`/recuo lateral), `.sobre-grid` e `.projeto-imagens` a uma coluna, recuos laterais `24px`/`16px`. **Falta na Fase 10:** revisar os paddings verticais de seção contra a tabela, o `scroll-margin-top` quando o menu quebra em duas linhas, e conferir cada componente da tabela 12.8 ponto a ponto.
