...
- **Arquivo de demonstração com dados fictícios (regra do autor, 2026-09-10):** quando o autor **confirmar explicitamente** que um arquivo de demonstração (captura ou vídeo) foi produzido com **dados fictícios** — os mesmos das telas dos cards, cobertos pela legenda "As telas usam dados fictícios, criados só para demonstração" —, **nomes de hóspedes e de funcionários nesse arquivo não bloqueiam a publicação**. Continuam **bloqueando sempre, mesmo em demonstração**: telas de login, nomes de usuário, senhas (mesmo mascaradas por bolinhas, o campo/rótulo de senha numa tela de login) e endereços/URLs/IPs/nome de servidor. Sem a confirmação do autor, todo nome de pessoa numa captura é tratado como dado real e bloqueia.
